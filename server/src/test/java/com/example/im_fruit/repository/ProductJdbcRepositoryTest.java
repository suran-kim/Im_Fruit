...
class ProductJdbcRepositoryTest {
    ...
    static void setup() {
        ...
        embeddedMysql = anEmbeddedMysql(config)
                .addSchema("test-order_fruit", ScriptResolver.classPathScript("schema.sql"))
                .start();
    }
    ...
    // 테스트를 위한 product 객체
    private final Product newProduct = new Product(UUID.randomUUID(), "new-product", Category.RANDOM_PACKAGES, 1000L); // static으로 생성해야 테스트마다 인스턴스가 생성되지 않는다.
